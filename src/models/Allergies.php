...
class Allergies extends Database {
    ...
    public function createAllergy($data){
        ...
        $sql = "INSERT INTO $this->tableName SET name='$name',created_by='$this->adminID',created_at='$time'";
        $this->query($sql);
        $this->execute();
        $id = $this->lastInsertId();
        
        $activity = "New allergy created.Id-".$id;
    ...
    public function updateAllergy($data)
    {
        ...
        $activity = "Updated allergy.Id-".$id;
        return $this->adminActivityLog($activity);
    ...
    public function getAllergiesList($data){
       ...
       if(!empty($data['export'])){
        ...
        $edit_permission   = in_array(84,$this->admin_services) || $this->admin_role == '1' ? true : false;
        ...
        foreach ($result['data'] as $key => $value) {
            ...
            $result['data'][$key]['action']  = $edit_permission ? '<a href="'.BASEURL.'Allergies/Edit?id='.base64_encode($value['id']).'"><button class="btn btn-primary">Edit</button></a>':'-';                             
    ...
    public function updateStatus($params)
    {
        ...
        $activity = "Admin changed the allergy status to ".$status.'.Id-'.$id;
        return $this->adminActivityLog($activity);
